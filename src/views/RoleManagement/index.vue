...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      add() {
        this.$router.push({name: 'AddRole', params: {title: '新增角色', data: null, show: true, disabled: false}})
      },
      handleClickInfo(row) {
        this.$router.push({name: 'AddRole', params: {title: '角色详情', data: row, show: false, disabled: true}})
      },
      handleClickModify(row) {
        this.$router.push({name: 'AddRole', params: {title: '编辑角色', data: row, show: true, disabled: false}})
      },
      modify() {
